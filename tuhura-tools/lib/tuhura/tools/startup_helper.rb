...
module Tuhura::Tools
  module StartupHelper
    ...
    def self._execute_task(task, args)
      ...
      if @@install_gems
        cmd = "cd #{task[:top_dir]}; env BUNDLE_GEMFILE=#{gemfile} rvm #{task[:ruby]} exec bundle install"
        puts ".. executing #{cmd}" if @@verbose
        @@verbose ? puts(`#{cmd}`) : `#{cmd}`
      end
      if @@update_gems
        cmd = "cd #{task[:top_dir]}; env BUNDLE_GEMFILE=#{gemfile} rvm #{task[:ruby]} exec bundle update"
        puts ".. executing #{cmd}" if @@verbose
        @@verbose ? puts(`#{cmd}`) : `#{cmd}`
      end
